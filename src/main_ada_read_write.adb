...
begin
   ...
   -- Delay to make closing files take effect  <==== IMPT
   ...
   while not ATIO.End_Of_File (inp_fhandle_02) loop
      ...
      if (inp_first3Chars = "G01") then
         out_countG01 := out_countG01 + 1;
      end if;
      
      if (inp_first3Chars = "G02") then
         out_countG02 := out_countG02 + 1;
      end if;
      
      if (inp_first3Chars = "G03") then
         out_countG03 := out_countG03 + 1;
      end if;
      
      if (inp_first3Chars = "G21") then
...
-- Opened inp_fhandle_02 as input
-- Opened out_fhandle_02 as output
...
-- 2021-02-24 08:10:49.14212553496 Alhamdulillah 3 times WRY
-- [2021-02-24 16:10:49] process terminated successfully, elapsed time: 10.26s
